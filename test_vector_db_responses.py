"""
Test script: run many commands against the Document Search API, capture all
responses, and store them in a file.

Usage:
    python test_vector_db_responses.py
    python test_vector_db_responses.py --base http://localhost:5003
...
def write_txt_report(results: list[dict], path: Path) -> None:
    lines = [
        "=" * 60,
        "Document Search (Jina Reranker) test responses",
    ...
    for r in results:
        ...
        if "results" in resp and isinstance(resp["results"], list):
            ...
            for i, item in enumerate(resp["results"][:3]):
                ...
                lines.append(
                    f"    [{i+1}] table={meta.get('table', '?')} score={score} content={content_preview!r}"
                )
...
def main():
    parser = argparse.ArgumentParser(description="Test Document Search API and save responses")
    parser.add_argument("--base", default="http://localhost:5003", help="Base URL of the API")
    parser.add_argument("--out", default=None, help="Base name for output files")
    ...
    print(f"Testing Document Search API at {base}")
